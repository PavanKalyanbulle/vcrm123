...
class Contacts_Module_Model extends Vtiger_Module_Model {
	...
	public function demoPicklistAjax($demoPick){
		global $adb;
		
		$dp = array("DP 1"=>1,"DP 2"=>5,"DP 3"=>10);
		
		$sqlDQ = "SELECT vtiger_users.id,CONCAT(vtiger_users.first_name,' ',vtiger_users.last_name) AS uname FROM vtiger_users WHERE vtiger_users.id=?";
		$dataDQ = array($dp[$demoPick]);
		$resDQ = $adb->pquery($sqlDQ,$dataDQ);
		$detDQ = array();
		$detDQ = $adb->fetchByAssoc($resDQ);
		if(!empty($detDQ['id'])){
			return $detDQ['uname'];
		}else{
