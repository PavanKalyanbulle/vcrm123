...
require_once("modules/Ageing/Ageing.php");
...
class CRMEntity {
	...
	function saveentity($module, $fileid = '') {
		...
		if($module == 'HelpDesk'){
			//for updating material return details
			$this->column_fields['cf_1087'] = $this->column_fields['cf_987'];
			$this->column_fields['cf_1089'] = $this->column_fields['cf_991'];
			$this->column_fields['cf_1091'] = $this->column_fields['cf_993'];
			
			//for updating total cost
			$costType = $this->column_fields['cf_999'];
			$quantity = $this->column_fields['cf_913'];
			$newCost = $this->column_fields['cf_1009'];
			$repairedCost = $this->column_fields['cf_1011'];
			if($costType == 'New'){
				$totalCost =  $quantity * $newCost; // quantity * new cost
			} else {
				$totalCost =  $quantity * $repairedCost; // quantity * repaired cost
			}
			$this->column_fields['cf_1045'] = $totalCost; //updating total cost
			
			//for updating doc path for old RCA
			if(!empty($this->column_fields['cf_1187'])){
					$rcatype = $this->column_fields['cf_1187'];
					if($rcatype == 'Old'){
						$rcano = $this->column_fields['cf_1187'];
						$sqlrca= "SELECT cf_1185 FROM vtiger_rcacf where rcaid= ?";
						$datarca= array($rcano);
						$resrca = $adb->pquery($sqlrca,$datarca);
						//echo $adb->convert2Sql($sqlrecid, $datarecid);
						$recdata = $adb->fetchByAssoc($resrca);	
						$rcaDocPath = $recdata['cf_1185'];
						$this->column_fields['cf_1171'] =  $rcaDocPath;
					}	
			}
		}
		...
		// ticket number format changing 2024-01-23
		if (strpos($this->column_fields['ticket_no'], 'IN') !== 0) {
			$ticketname = 'IN';  //for India
			
			if ($this->column_fields['cf_929'] == 'EAST') {
				$ticketname .= 'EA';
			} elseif ($this->column_fields['cf_929'] == 'WEST') {
				$ticketname .= 'WE';
			} elseif ($this->column_fields['cf_929'] == 'NORTH') {
				$ticketname .= 'NO';
			} elseif ($this->column_fields['cf_929'] == 'SOUTH') {
				$ticketname .= 'SO';
			}
			
			if ($this->column_fields['cf_903'] == 'Verbal') {
				$ticketname .= 'V';
			} elseif ($this->column_fields['cf_903'] == 'Letter') {
				$ticketname .= 'L';
			} elseif ($this->column_fields['cf_903'] == 'Warranty Rejection') {
				$ticketname .= 'E';
			}
			
			$currentyear = date('Y');
			$ticketname .= $currentyear;
			
			$ticketNumber = preg_replace('/[^0-9]/', '', $this->column_fields['ticket_no']);
			$formattedTicketNumber = sprintf('%04d', $ticketNumber);
			$ticketname .= $formattedTicketNumber;
			
			$sqltn = "UPDATE vtiger_troubletickets SET ticket_no=? WHERE ticket_no=?";
			$datatn = array($ticketname , $this->column_fields['ticket_no']);
			$restn = $adb->pquery($sqltn,$datatn);
			$this->column_fields['ticket_no'] = $ticketname;
		}		
		// End script to modify ticket number
		 
		//code to update status of ticket
		if($module == 'HelpDesk'){
			$this->updateStatusOfTicket($columnFields);
		}
	...
	public function updateStatusOfTicket($columnFields)
	{
		global $adb;

		$ticket_no = $columnFields['ticket_no'];
		$modifiedDate = date('Y-m-d', strtotime($columnFields['modifiedtime']));
		$railwayClosure = $columnFields['cf_1201'];
		$mrnno = $columnFields['cf_947'];
		$seapproval = $columnFields['cf_947'];
		$materialrequired = $columnFields['cf_951'];
		$rmapproval = $columnFields['cf_1031'];
		$cost = $columnFields['cf_1045'];
		$hodapproval = $columnFields['cf_1035'];
		$coapproval = $columnFields['cf_1039'];
		$materialreturn = $columnFields['cf_1079'];
		$rcano = $columnFields['cf_1169'];
		$DOnumber = $columnFields['cf_1077'];
		$asnToSe = $columnFields['cf_1105'];

		$curr_stage = $this->getCurrentStage($ticket_no);

		if (empty($curr_stage)) {
			$stage = $asnToSe == 'Yes' ? '2 - SE Verification' : '1 - Ticket Creation';
			$this->addNewAgeing($adb, $stage, $columnFields);
			$this->column_fields['cf_1215'] = $stage;
		} else {
			...
			switch ($curr_stage) {
				case '1 - Ticket Creation':
					if ($asnToSe == 'Yes') {
						$fieldid = 'cf_1059';
						$this->updateStage($ticket_no, '2 - SE Verification', $modifiedDate,$fieldid);
					}
					break;
				case '2 - SE Verification':
					$this->handleSEVerification($columnFields, $modifiedDate, $railwayClosure);
					break;
				case '3 - DO Approval':
						$approved = $this->checkApproval($ticket_no,$cost,$rmapproval, $hodapproval, $coapproval,$modifiedDate);
						if ($approved && empty($materialreturn)) {
							$fieldid = 'cf_1061';//Do approval
							$this->updateStage($ticket_no, '4 - DO Punching', $modifiedDate ,$fieldid);
						}
					break;	
				case '4 - DO Punching':
					if (!empty($DOnumber) && $DOnumber != 'null' && empty($materialreturn)) {
						$fieldid = 'cf_1063';
						$this->updateStage($ticket_no, '5 - PPC Pending for readiness', $modifiedDate,$fieldid);
					}
					break;
				case '8 - Material Delivered':
					if ($materialreturn == 'No') {
						$this->assignToQuality($columnFields);
						$fieldid = 'cf_1069';
						$this->updateStage($ticket_no, '10 - RCA/CAPA', $modifiedDate,$fieldid);
					}
					if (!empty($railwayClosure) && $railwayClosure == 'Yes') {
						$sqlRC = "UPDATE vtiger_ageingcf INNER JOIN vtiger_ageing ON vtiger_ageingcf.ageingid = vtiger_ageing.ageingid SET vtiger_ageingcf.cf_1205 = ?,vtiger_ageingcf.cf_1207 = ? WHERE vtiger_ageing.ticketno = ?";
						$data = array($modifiedDate,$modifiedDate, $ticket_no);
						$adb->pquery($sqlRC, $data);
					}
					break;
				case '9 - Material Returning':
					if (!empty($mrnno)) {
						$this->assignToQuality($columnFields);
						$fieldid = 'cf_1071';
						$this->updateStage($ticket_no, '10 - RCA/CAPA', $modifiedDate,$fieldid);
				...
				case '10 - RCA/CAPA':	
					...
					if (!empty($rcano)) {
						$fieldid = 'cf_1065';
						$this->updateStage($ticket_no, '11 - Ticket Closure', $modifiedDate,$fieldid);
					}
					break;
			}
	...
	public function getCurrentStage($ticket_no)
	{
		global $adb;
		$sqlAg = "SELECT vtiger_ageingcf.cf_1073 as curr_stage FROM vtiger_ageing INNER JOIN vtiger_ageingcf ON vtiger_ageingcf.ageingid = vtiger_ageing.ageingid WHERE vtiger_ageing.ticketno = ?";
		$dataAg = array($ticket_no);
		$resAg = $adb->pquery($sqlAg, $dataAg);

		if ($adb->num_rows($resAg) > 0) {
			$agdata = $adb->fetchByAssoc($resAg);
			return $agdata['curr_stage'];
		}
		return null;
	}

	public function updateStage($ticket_no, $stage, $modifiedDate,$fieldid)
	{
		global $adb;
		$sql = "UPDATE vtiger_ageingcf INNER JOIN vtiger_ageing ON vtiger_ageingcf.ageingid = vtiger_ageing.ageingid SET vtiger_ageingcf.cf_1073 = ?,  vtiger_ageingcf.$fieldid = ? WHERE vtiger_ageing.ticketno = ?";
		$data = array($stage, $modifiedDate, $ticket_no);
		$this->column_fields['cf_1215'] = $stage;
		//echo $adb->convert2Sql($sql,$data); die;
		$adb->pquery($sql, $data);
	}

	public function handleSEVerification($columnFields, $modifiedDate, $railwayClosure)
	{
		global $adb;
		$ticket_no = $columnFields['ticket_no'];
		$seapproval = $columnFields['cf_947'];
		...
		if ($seapproval == 'Yes' && $rmapproval != 'Yes') {
			
			if ($materialrequired == 'Required') {
				$fieldid = 'cf_1059';
				$this->updateStage($ticket_no, '3 - DO Approval', $modifiedDate ,$fieldid);
			} elseif ($materialrequired == 'Not Required') {
				$this->assignToQuality($columnFields);
				$fieldid = 'cf_1207';
				$this->updateStage($ticket_no, '10 - RCA/CAPA', $modifiedDate ,$fieldid);
	...
	public function checkApproval($ticket_no,$cost,$rmapproval, $hodapproval, $coapproval,$modifiedDate)
	{
		global $adb;
		$sql = "SELECT * FROM vtiger_ageing INNER JOIN vtiger_ageingcf ON vtiger_ageingcf.ageingid =vtiger_ageing.ageingid WHERE vtiger_ageing.ticketno =? ";
		$data = array($ticket_no);
		$res = $adb->pquery($sql,$data);
		$deta = $adb->fetchByassoc($res);

		if ($cost < 50000 && $rmapproval == 'Yes') {
			if(empty($deta['cf_1223'])){
				$fieldid = 'cf_1223';
				$this->updateageing($ticket_no, $modifiedDate,$fieldid);
			}
			return true;
		} elseif ($cost >= 50000 && $cost < 100000 && $hodapproval == 'Yes') {
			if(empty($deta['cf_1225'])){
				$fieldid = 'cf_1225';
				$this->updateageing($ticket_no, $modifiedDate,$fieldid);
			}
			return true;
		} elseif ($cost >= 100000 && $coapproval == 'Yes') {
			if(empty($deta['cf_1227'])){
				$fieldid = 'cf_1227';
				$this->updateageing($ticket_no, $modifiedDate,$fieldid);
			}
			return true;
	...
	public function updateageing($ticket_no,$modifiedDate,$fieldid){
		global $adb;
		$sql = "UPDATE vtiger_ageingcf INNER JOIN vtiger_ageing ON vtiger_ageingcf.ageingid = vtiger_ageing.ageingid SET   vtiger_ageingcf.$fieldid = ? WHERE vtiger_ageing.ticketno = ?";
		$data = array( $modifiedDate, $ticket_no);
		//echo $adb->convert2Sql($sql,$data); die;
		$adb->pquery($sql, $data);
	}

	public function addNewAgeing($adb, $stage, $column_fields)
	{
		$ticket_no = $column_fields['ticket_no'];
		$createdtime = date('Y-m-d', strtotime($column_fields['createdtime']));
		$sqlrecid = "SELECT ticketid FROM vtiger_troubletickets WHERE ticket_no = ?";
		$datarecid = array($ticket_no);
		$resreid = $adb->pquery($sqlrecid, $datarecid);
		$recdata = $adb->fetchByAssoc($resreid);
		$recordid = $recdata['ticketid'];

		$Ageing = new Ageing();
		$Ageing->column_fields['ticketno'] = $ticket_no;
		$Ageing->column_fields['assigned_user_id'] = '1';
		$Ageing->column_fields['cf_1073'] = $stage;
		$Ageing->column_fields['cf_1173'] = $recordid;
		$Ageing->column_fields['cf_1199'] = $createdtime;
		$Ageing->saveentity('Ageing');

		$sqlagid = "SELECT ageingid FROM vtiger_ageing WHERE ticketno = ?";
		$dataagid = array($ticket_no);
		$resagid = $adb->pquery($sqlagid, $dataagid);
		$agdata = $adb->fetchByAssoc($resagid);
		$agid = $agdata['ageingid'];

		$sqlrelag = "INSERT INTO vtiger_crmentityrel (crmid, module, relcrmid, relmodule) VALUES (?, ?, ?, ?)";
		$dataag = array($recordid, 'HelpDesk', $agid, 'Ageing');
		$adb->pquery($sqlrelag, $dataag);
	}

	public function assignToQuality($column_fields)
	{
		global $adb;
		$ticketno = $column_fields['ticket_no'];
		$sqlq = "SELECT vtiger_ticketcf.cf_989 as product_desc FROM vtiger_ticketcf INNER JOIN vtiger_troubletickets ON vtiger_troubletickets.ticketid = vtiger_ticketcf.ticketid WHERE vtiger_troubletickets.ticket_no = ?";
		$dataq = array($ticketno);
		$qdata = $adb->pquery($sqlq, $dataq);
		$pData = $adb->fetchByAssoc($qdata);
		$productDesc = $pData['product_desc'];
		
		if(empty($productDesc)){
			$productDesc = 'ALL';
		}
		if (!empty($productDesc)) {
			$sqlu = "SELECT id FROM vtiger_users INNER JOIN vtiger_user2role ON vtiger_user2role.userid = vtiger_users.id WHERE product_code LIKE ? AND vtiger_user2role.roleid = ?";
			$datau = array('%' . $productDesc . '%', 'H15');
			$uresult = $adb->pquery($sqlu, $datau);
			$userData = $adb->fetchByAssoc($uresult);
			$userid = $userData['id'];

			if (!empty($userid)) {
				$sqlauid = "UPDATE vtiger_crmentity INNER JOIN vtiger_troubletickets ON vtiger_troubletickets.ticketid = vtiger_crmentity.crmid SET vtiger_crmentity.smownerid = ? WHERE vtiger_troubletickets.ticket_no = ?";
				$dataauid = array($userid, $ticketno);
				$adb->pquery($sqlauid, $dataauid);
			}
		}
	}
	...
	function uploadAndSaveFile($id, $module, $file_details, $attachmentType='Attachment') {
		...
		if($module == "HelpDesk"){
			if(!empty($_FILES['cf_958']['name'])){
				
				$uploadStatus = "";
				$fileUrl = ""; 
				$ticketid ='';
				$maindirname = $root_directory."document/";
				if(!file_exists($maindirname)) {
					mkdir($maindirname, 0777);
				}
				if(!empty($id)){
					$sqlT = "SELECT ticketid from vtiger_troubletickets  WHERE ticket_no=?";
					$dataT = array($this->column_fields['ticket_no']);
					$resT = $adb->pquery($sqlT,$dataT);
					$data = $adb->fetchByAssoc($resT);
					//echo $adb->convert2sql($sqlT,$dataT);die;
					$ticketid = $data['ticketid'];
				}
			
				$filedir = $maindirname.$ticketid;
				if(!file_exists($filedir)) {
					mkdir($filedir, 0777);
				}
			
				if(!empty($_FILES['cf_958']['name'])){
					$filename = $_FILES['cf_958']['name'];
					$fileurl = $filedir."/".$filename;
					if(move_uploaded_file($_FILES['cf_958']['tmp_name'], $fileurl)){
						$uploadStatus = "Uploaded";
					}
				}
				if(!empty($uploadStatus)){
					$sqlfile = "UPDATE vtiger_ticketcf SET cf_958=? WHERE ticketid=?";
					$datafile = array($fileurl,$ticketid);
					$resfile = $adb->pquery($sqlfile,$datafile);
				}
			}
			if(!empty($_FILES['cf_1097']['name'])){
				...
				if(!empty($_FILES['cf_1097']['name'])){
					$filename = $_FILES['cf_1097']['name'];
					$fileurl = $filedir."/".$filename;
					if(move_uploaded_file($_FILES['cf_1097']['tmp_name'], $fileurl)){
						$uploadStatus = "Uploaded";
					}
				}
				if(!empty($uploadStatus)){
					$sqlfile = "UPDATE vtiger_ticketcf SET cf_1097=? WHERE ticketid=?";
					$datafile = array($fileurl,$ticketid);
					$resfile = $adb->pquery($sqlfile,$datafile);
				}
			}
			if(!empty($_FILES['cf_1081']['name'])){
				...
				if(!empty($_FILES['cf_1081']['name'])){
					$filename = $_FILES['cf_1081']['name'];
					$fileurl = $filedir."/".$filename;
					if(move_uploaded_file($_FILES['cf_1081']['tmp_name'], $fileurl)){
						$uploadStatus = "Uploaded";
					}
				}
				if(!empty($uploadStatus)){
					$sqlfile = "UPDATE vtiger_ticketcf SET cf_1081=? WHERE ticketid=?";
					$datafile = array($fileurl,$ticketid);
					$resfile = $adb->pquery($sqlfile,$datafile);
				}
			}
			
			if(!empty($_FILES['cf_1171']['name'])){
				...
				if(!empty($_FILES['cf_1171']['name'])){
					$filename = $_FILES['cf_1171']['name'];
					$fileurl = $filedir."/".$filename;
					if(move_uploaded_file($_FILES['cf_1171']['tmp_name'], $fileurl)){
						$uploadStatus = "Uploaded";
					}
				}
				
				$record_id = $this->column_fields['record_id'];
				if(!empty($uploadStatus)){
					$sqlfile = "UPDATE vtiger_rcacf  INNER JOIN vtiger_rca ON vtiger_rca.rcaid = vtiger_rcacf.rcaid SET cf_1185=? WHERE vtiger_rca.ticketnumber=?";
					$datafile = array($fileurl,$record_id);
					$resfile = $adb->pquery($sqlfile,$datafile);
				}
				
				if(!empty($uploadStatus)){
					$sqlfile = "UPDATE vtiger_ticketcf SET cf_1171=? WHERE ticketid=?";
					$datafile = array($fileurl,$ticketid);
					$resfile = $adb->pquery($sqlfile,$datafile);
				}
			}
