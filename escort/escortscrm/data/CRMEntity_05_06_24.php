		...
		if($mode == 'edit'){	
			...
			if($seapproval == 'Yes' && $rmapproval != 'Yes'){	
				...
				if(!empty($DOnumber) && $DOnumber!='null' && empty($materialreturn)){
					...
					$data = array($modifiedDate,$stage,$ticket_no);
					$resDO = $adb->pquery($sqlDO,$data);
				}else if($approved && !empty($rcano)){
					$stage= '10 - Ticket Closure ';
			...
			$datarecid = array($this->column_fields['ticket_no']);
