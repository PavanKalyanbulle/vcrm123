...
class CRMEntity {
	...
	public function updateStatusOfTicket($columnFields){	
		...
		if($mode == 'edit'){	
			...
			$seapproval = $columnFields['cf_947']; $materialrequired = $columnFields['cf_951'];$rmapproval = $columnFields['cf_1031'];
			if($seapproval == 'Yes' && $rmapproval == 'No'){	
				if($materialrequired == 'Required'){
					$stage = '3 - DO Approval';
					$sqlSE = "UPDATE vtiger_ageingcf INNER JOIN vtiger_ageing ON vtiger_ageing.ageingid = vtiger_ageingcf.ageingid SET vtiger_ageingcf.cf_1059=?, vtiger_ageingcf.cf_1073=? WHERE vtiger_ageing.ticketno=?";
					$data = array($modifiedDate,$stage,$ticket_no);
					$resSE = $adb->pquery($sqlSE,$data);
				}else{
					$stage = '10 - RCA/CAPA';	
					//$this->assignToQuality($columnFields);
					$sql = "UPDATE vtiger_ageingcf INNER JOIN vtiger_ageing ON vtiger_ageing.ageingid = vtiger_ageingcf.ageingid SET vtiger_ageingcf.cf_1207=?, vtiger_ageingcf.cf_1073=? WHERE vtiger_ageing.ticketno=?";
					$data = array($modifiedDate,$stage,$ticket_no);
					$res = $adb->pquery($sql,$data);
				}			
			}else{
				
				$cost = $columnFields['cf_1045']; $hodapproval = $columnFields['cf_1035']; $coapproval = $columnFields['cf_1039'];
				$materialreturn = $columnFields['cf_1079'];   $rcano =  $columnFields['cf_1169'];
				$approved = false; $DOnumber = $columnFields['cf_1077'];
				if($cost<50000 && $rmapproval=='Yes' ){
					$approved =true;
				}else if($cost>50000 && $cost<100000 &&  $hodapproval=='Yes'){
					$approved =true;
				}else if($cost>100000 && $coapproval=='Yes'){
					$approved =true;
				}
				
				if($approved && !empty($rcano)){
					$stage= '11 - Ticket Closure';
					$sql = "UPDATE vtiger_ageingcf  INNER JOIN vtiger_ageing ON vtiger_ageingcf.ageingid =vtiger_ageing.ageingid SET vtiger_ageingcf.cf_1065=?,vtiger_ageingcf.cf_1073=?  WHERE vtiger_ageing.ticketno=?";
					$data = array($modifiedDate,$stage,$ticket_no);
					$res = $adb->pquery($sql,$data);
				}else if(!empty($mrnno)){
					$stage = '10 - RCA/CAPA';
					...
					$res = $adb->pquery($sql,$data);
					$this->assignToQuality($columnFields);	
				}else if(!empty($railwayClosure) && $railwayClosure == 'Yes'){		
					$sqlRC = "UPDATE vtiger_ageingcf  INNER JOIN vtiger_ageing ON vtiger_ageingcf.ageingid =vtiger_ageing.ageingid SET vtiger_ageingcf.cf_1205=? WHERE vtiger_ageing.ticketno=?";
					$data = array($modifiedDate,$ticket_no);
					$resRC = $adb->pquery($sqlDO,$data);	
				}else if(!empty($DOnumber) && $DOnumber!='null' && empty($materialreturn)){
					$stage= '5 - PPC Pending for readiness';
					$sqlDO = "UPDATE vtiger_ageingcf  INNER JOIN vtiger_ageing ON vtiger_ageingcf.ageingid =vtiger_ageing.ageingid SET vtiger_ageingcf.cf_1063=?,vtiger_ageingcf.cf_1073=?  WHERE vtiger_ageing.ticketno=?";
					$data = array($modifiedDate,$stage,$ticket_no);
					$resDO = $adb->pquery($sqlDO,$data);
				}else if($approved && empty($materialreturn)){
					$stage= '4 - DO Punching';
					$sqlDO = "UPDATE vtiger_ageingcf  INNER JOIN vtiger_ageing ON vtiger_ageingcf.ageingid =vtiger_ageing.ageingid SET vtiger_ageingcf.cf_1061=?,vtiger_ageingcf.cf_1073=?  WHERE vtiger_ageing.ticketno=?";
					$data = array($modifiedDate,$stage,$ticket_no);
					$resDO = $adb->pquery($sqlDO,$data);
				}else if($asnToSe == 'Yes' && $rmapproval !='Yes'){
					$stage= '2 - SE Verification';
					$sql = "UPDATE vtiger_ageingcf  INNER JOIN vtiger_ageing ON vtiger_ageingcf.ageingid =vtiger_ageing.ageingid SET vtiger_ageingcf.cf_1059=?,vtiger_ageingcf.cf_1073=?  WHERE vtiger_ageing.ticketno=?";
					$data = array($modifiedDate,$stage,$ticket_no);
					$res = $adb->pquery($sql,$data);
				}
			}	
				...
				if($asnToSe == 'Yes')
					...
					$stage= '1 - Ticket Creation';
				$this->addNewAgeing($adb,$stage ,$this->column_fields);
		} 
	}
	
	function addNewAgeing($adb,$stage ,$column_fields ){
			
			$ticket_no = $column_fields['ticket_no'];
			$createdtime = date('Y-m-d', strtotime($columnFields['createdtime']));
			$sqlrecid = "SELECT ticketid FROM vtiger_troubletickets where ticket_no= ?";
			$datarecid = array($column_fields['ticket_no']);
			...
			if($stage == '1 - Ticket Creation' ){
				$Ageing->column_fields['cf_1199'] = $recordid;
			}else if($stage == '2 - SE Verification'){
				$Ageing->column_fields['cf_1199'] = $recordid;
			}
			$Ageing->saveentity('Ageing');

			$sqlagid = "SELECT ageingid FROM vtiger_ageing where ticketno= ?";
			$dataagid = array($column_fields['ticket_no']);
			...
			$resin = $adb->pquery($sqlrelag,$dataag);
	}
	
	function assignToQuality($column_fields){
		...
		//echo $adb->convert2Sql($sqlq, $dataq);
		...
		if(!empty($productDesc)){
			...
			//echo $adb->convert2Sql($sqlu, $datau); 
			$uresult = $adb->pquery($sqlu, $datau); 
			$userData = $adb->fetchByAssoc($uresult);
			$userid = $userData['id'];
			
			if(!empty($userid)){
				$sqlauid = "UPDATE vtiger_crmentity INNER JOIN vtiger_troubletickets ON vtiger_troubletickets.ticketid = vtiger_crmentity.crmid 
									SET vtiger_crmentity.smownerid = ? WHERE vtiger_troubletickets.ticket_no = ?";
				$dataauid = array($userid, $ticketno);
				//echo $adb->convert2Sql($sqlauid, $dataauid); die;
				$res = $adb->pquery($sqlauid, $dataauid);
			}
		}
	}
