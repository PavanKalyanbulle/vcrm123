...
class CustomModule_Index_View extends Vtiger_Index_View
{
    ...
    function process(Vtiger_Request $request){
		...
		// while($detApp=$adb->fetchByAssoc($resApp)){
			...
			// }
		// }
		$pmList = array('SALES','MARKETING','INVENTORY','INVSTAT','SUPPORT','PROJECT','TOOLS','ADVTOOLS','PAPERROLL');
		$appList = '<option value="">Select an App</option>';
		for($pmc=0;$pmc<count($pmList);$pmc++){
			$appList .= '<option value="'.$pmList[$pmc].'">'.$pmList[$pmc].'</option>';
		}
